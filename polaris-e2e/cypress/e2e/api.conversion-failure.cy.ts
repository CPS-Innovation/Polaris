...
let docs: PresentationDocumentProperties[];

const getNameFromDocId = (docId: string) =>
  docs.find((doc) => doc.documentId === docId).cmsOriginalFileName;
    ...
    cy.clearCaseTracker(CONVERSION_FAILURE_CASE_URN, CONVERSION_FAILURE_CASE_ID)
      ...
      .then(({ documents }) => {
        expect(
          documents.some(
            (document) =>
              getNameFromDocId(document.documentId) ===
                CONVERSION_FAILURE_PASSWORD_PROTECTED_DOCUMENT_NAME &&

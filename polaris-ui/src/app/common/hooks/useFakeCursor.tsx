    ...
    (e: KeyboardEvent) => {
      if (activeTabId === tabId) {
        ...
        if (
          ...
          // if (cursor) {
          //   triggerMouseDblClick();
          // }
          const textLayer = document.querySelectorAll(".textLayer")[0];
          if (activeButtonIndex.current < textLayer.children.length) {
            activeButtonIndex.current = activeButtonIndex.current + 1;
          }
          while (
            !textLayer.children[activeButtonIndex.current].textContent?.trim()
          ) {
            if (activeButtonIndex.current < textLayer.children.length)
              activeButtonIndex.current = activeButtonIndex.current + 1;
            else {
          ...
          console.log(
            "hiiii",
            textLayer.children[activeButtonIndex.current].textContent
          );
          (
            textLayer.children[activeButtonIndex.current] as HTMLElement
          ).focus();
          const child = textLayer.children[activeButtonIndex.current];
          child.dispatchEvent(
            new MouseEvent("mousedown", {
              bubbles: true,
            })

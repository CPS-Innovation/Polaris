...
resource "azurerm_windows_function_app_slot" "fa_pdf_thumbnail_generator_staging" {
  ...
  app_settings = {
    ...
    "SlidingClearDownInputHours"                      = var.thumbnail_generator_sliding_clear_down.input_hours
    "BlobServiceContainerNameThumbnails"              = var.blob_thumbnails_container_name
    "ThumbnailGeneratorTaskHub"                       = "fapolaris${var.env != "prod" ? var.env : ""}pdfthumbnailgeneratorstaging"
    "FUNCTIONS_EXTENSION_VERSION"                     = "~4"
    "FUNCTIONS_WORKER_RUNTIME"                        = "dotnet-isolated"
    "HostType"                                        = "Staging"
    "SCALE_CONTROLLER_LOGGING_ENABLED"                = var.pipeline_logging.pdf_thumbnail_generator_scale_controller
    ...
    "WEBSITE_CONTENTAZUREFILECONNECTIONSTRING"        = azurerm_storage_account.sa_pdf_thumbnail_generator.primary_connection_string
    "WEBSITE_CONTENTOVERVNET"                         = "1"
    "WEBSITE_CONTENTSHARE"                            = azapi_resource.pipeline_sa_pdf_thumbnail_generator_file_share_staging.name
    "WEBSITE_DNS_ALT_SERVER"                          = var.dns_alt_server
    "WEBSITE_DNS_SERVER"                              = var.dns_server
    "WEBSITE_ENABLE_SYNC_UPDATE_SITE"                 = "1"
    "WEBSITE_OVERRIDE_STICKY_DIAGNOSTICS_SETTINGS"    = "0"
...
# Create Private Endpoint
resource "azurerm_private_endpoint" "pipeline_pdf_thumbnail_generator_staging_pe" {
  name                = "${azurerm_windows_function_app.fa_pdf_thumbnail_generator.name}-staging-pe"
  resource_group_name = azurerm_resource_group.rg_polaris_pipeline.name
  location            = azurerm_resource_group.rg_polaris_pipeline.location
  subnet_id           = data.azurerm_subnet.polaris_apps2_subnet.id
  tags                = local.common_tags
  ...
  private_service_connection {
    name                           = "${azurerm_windows_function_app.fa_pdf_thumbnail_generator.name}-staging-psc"
    private_connection_resource_id = azurerm_windows_function_app.fa_pdf_thumbnail_generator.id
    is_manual_connection           = false

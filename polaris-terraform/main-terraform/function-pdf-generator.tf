...
resource "azurerm_windows_function_app" "fa_pdf_generator" {
  name                          = "fa-${local.global_resource_name}-pdf-generator"
  ...
  /*app_role = [
    {
      ...
      description          = "Can create PDF resources using the ${local.pipeline_resource_name} PDF Generator"
...
resource "azurerm_role_assignment" "ra_blob_delegator_pdf_generator" {
  ...
  depends_on = [
    ...
    azurerm_windows_function_app.fa_pdf_generator
  ]
}

...
resource "azurerm_linux_function_app" "fa_polaris_maintenance" {
  ...
  name                          = "fa-${local.global_resource_name}-maintenance"
...
module "azurerm_app_reg_fa_maintenance" { # Note, app roles are currently being managed outside of terraform and it's functionality has been commented out from the module.
  ...
  display_name            = "fa-${local.global_resource_name}-maintenance-appreg"
  identifier_uris         = ["api://fa-${local.global_resource_name}-maintenance"]

...
resource "azurerm_service_plan" "asp_polaris_pipeline_coordinator" {
  ...
  location            = var.env == "prod" ? azurerm_resource_group.rg_mv[0].location : azurerm_resource_group.rg.location
  resource_group_name = var.env == "prod" ? azurerm_resource_group.rg_mv[0].name : azurerm_resource_group.rg.name
  os_type             = "Linux"

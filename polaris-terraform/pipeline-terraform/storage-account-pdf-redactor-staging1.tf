resource "azurerm_storage_account" "sa_pdf_redactor_staging1" {
  #checkov:skip=CKV_AZURE_206:Ensure that Storage Accounts use replication
  #checkov:skip=CKV2_AZURE_38:Ensure soft-delete is enabled on Azure storage account
  #checkov:skip=CKV2_AZURE_1:Ensure storage for critical data are encrypted with Customer Managed Key
  #checkov:skip=CKV2_AZURE_21:Ensure Storage logging is enabled for Blob service for read requests
  #checkov:skip=CKV2_AZURE_40:Ensure storage account is not configured with Shared Key authorization
  name                = "sacps${var.env != "prod" ? var.env : ""}pdfredactor2"
...
resource "azurerm_private_endpoint" "pipeline_sa_pdf_redactor_staging1_blob_pe" {
  name                = "sacps${var.env != "prod" ? var.env : ""}pdfredactor-staging1-blob-pe"
  resource_group_name = azurerm_resource_group.rg.name
  location            = azurerm_resource_group.rg.location
  ...
  private_service_connection {
    name                           = "sacps${var.env != "prod" ? var.env : ""}pdfredactor-staging1-blob-psc"
    private_connection_resource_id = azurerm_storage_account.sa_pdf_redactor_staging1.id
...
resource "azurerm_private_endpoint" "pipeline_sa_pdf_redactor_staging1_table_pe" {
  name                = "sacps${var.env != "prod" ? var.env : ""}pdfredactor-staging1-table-pe"
  resource_group_name = azurerm_resource_group.rg.name
  location            = azurerm_resource_group.rg.location
  subnet_id           = data.azurerm_subnet.polaris_apps2_subnet.id
  tags                = local.common_tags
  ...
  private_service_connection {
    name                           = "sacps${var.env != "prod" ? var.env : ""}pdfredactor-staging1-table-psc"
    private_connection_resource_id = azurerm_storage_account.sa_pdf_redactor_staging1.id
    is_manual_connection           = false
...
resource "azurerm_private_endpoint" "pipeline_sa_pdf_redactor_staging1_file_pe" {
  name                = "sacps${var.env != "prod" ? var.env : ""}pdfredactor-staging1-file-pe"
  resource_group_name = azurerm_resource_group.rg.name
  location            = azurerm_resource_group.rg.location
  subnet_id           = data.azurerm_subnet.polaris_apps2_subnet.id
  tags                = local.common_tags
  ...
  private_service_connection {
    name                           = "sacps${var.env != "prod" ? var.env : ""}pdfredactor-staging1-file-psc"
    private_connection_resource_id = azurerm_storage_account.sa_pdf_redactor_staging1.id
    is_manual_connection           = false
...
resource "azurerm_private_endpoint" "pipeline_sa_pdf_redactor_staging1_queue_pe" {
  name                = "sacps${var.env != "prod" ? var.env : ""}pdfredactor-staging1-queue-pe"
  resource_group_name = azurerm_resource_group.rg.name
  location            = azurerm_resource_group.rg.location
  subnet_id           = data.azurerm_subnet.polaris_apps2_subnet.id
  tags                = local.common_tags
  ...
  private_service_connection {
    name                           = "sacps${var.env != "prod" ? var.env : ""}pdfredactor-staging1-queue-psc"
    private_connection_resource_id = azurerm_storage_account.sa_pdf_redactor_staging1.id
    is_manual_connection           = false

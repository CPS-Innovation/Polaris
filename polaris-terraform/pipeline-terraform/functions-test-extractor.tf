...
resource "azurerm_linux_function_app" "fa_test_extractor" {
  name                          = "fa-${local.global_name}-test-extractor"
  location                      = azurerm_resource_group.rg.location
  resource_group_name           = azurerm_resource_group.rg.name
  service_plan_id               = azurerm_service_plan.asp_polaris_ep_test_extractor.id
  ...
  app_settings = {
    ...
    "WEBSITE_CONTENTSHARE"                            = azapi_resource.pipeline_sa_test_extractor_file_share.name
...
module "azurerm_app_reg_fa_test_extractor" {
  source                  = "./modules/terraform-azurerm-azuread-app-registration"
  display_name            = "fa-${local.global_name}-test-extractor-appreg"
  identifier_uris         = ["api://fa-${local.global_name}-test-extractor"]
...
resource "azuread_application_password" "faap_fa_test_extractor_app_service" {
  application_object_id = module.azurerm_app_reg_fa_test_extractor.object_id
  end_date_relative     = "17520h"
}

module "azurerm_service_principal_fa_test_extractor" {
  source                       = "./modules/terraform-azurerm-azuread_service_principal"
  application_id               = module.azurerm_app_reg_fa_test_extractor.client_id
  app_role_assignment_required = false
  owners                       = [data.azurerm_client_config.current.object_id]
}

resource "azuread_service_principal_password" "sp_fa_test_extractor_pw" {
  service_principal_id = module.azurerm_service_principal_fa_test_extractor.object_id
}

resource "azuread_service_principal_delegated_permission_grant" "polaris_test_extractor_grant_access_to_msgraph" {
  service_principal_object_id          = module.azurerm_service_principal_fa_test_extractor.object_id

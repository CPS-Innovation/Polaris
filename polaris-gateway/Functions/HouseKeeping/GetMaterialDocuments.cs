...
/// </summary>
/// <remarks>
/// Initializes a new instance of the <see cref="GetMaterialDocument"/> class.
/// </remarks>
...
public class GetMaterialDocument(
    ILogger<GetMaterialDocument> logger,
    ICommunicationService communicationService,
    IDocumentService documentService) : BaseFunction(logger)
{
    private readonly ILogger<GetMaterialDocument> logger = logger;
    ...
    [OpenApiOperation(operationId: nameof(GetMaterialDocument), tags: ["Material"], Description = "Represents a function that retrieves the case material document for display purposes.")]
    ...
    public async Task<IActionResult> Run([HttpTrigger(AuthorizationLevel.Anonymous, "get", Route = RestApi.MaterialDocument)] HttpRequest req, int caseId, int materialId)
    {
        try
        {
            ...
            this.logger.LogInformation($"{LoggingConstants.HskUiLogPrefix} GetMaterialDocument function processed a request.");
        ...
        {
            this.logger.LogError($"{LoggingConstants.HskUiLogPrefix} GetMaterialDocument function encountered an error: {ex.Message}");
    ...
    private async Task<IActionResult> GetMaterialDocumentAsync(string caseId, string link, CmsAuthValues cmsAuthValues, Stopwatch stopwatch)
    {
        ...
        this.logger.LogInformation($"{LoggingConstants.HskUiLogPrefix} Milestone: caseId [{caseId}] GetMaterialDocument function completed in [{stopwatch.Elapsed}]");

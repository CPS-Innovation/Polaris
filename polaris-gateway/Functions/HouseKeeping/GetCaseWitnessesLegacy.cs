// <copyright file="GetCaseWitnessesLegacy.cs" company="TheCrownProsecutionService">
...
public class GetCaseWitnessesLegacy(ILogger<GetCaseWitnessesLegacy> logger,
    IWitnessService witnessService) : BaseFunction(logger)
{
    private readonly ILogger<GetCaseWitnessesLegacy> logger = logger;
    ...
    [OpenApiOperation(operationId: nameof(GetCaseWitnessesLegacy), tags: ["Witness"], Description = "Represents a function that retrieves witnesses for a case.")]
    ...
    [Function(nameof(GetCaseWitnessesLegacy))]
    public async Task<IActionResult> Run([HttpTrigger(AuthorizationLevel.Anonymous, "get", Route = RestApi.CaseWitnessesHkLegacy)] HttpRequest request, int caseId, CancellationToken cancellationToken = default)
    {
        try
        {
            var stopwatch = Stopwatch.StartNew();
            logger.LogInformation($"{LoggingConstants.HskUiLogPrefix} {nameof(GetCaseWitnessesLegacy)} function processed a request.");
            ...
            logger.LogInformation($"{LoggingConstants.HskUiLogPrefix} Milestone: caseId [{caseId}] {nameof(GetCaseWitnessesLegacy)} function completed in [{stopwatch.Elapsed}]");
        ...
        {
            logger.LogError($"{LoggingConstants.HskUiLogPrefix} {nameof(GetCaseWitnessesLegacy)} function encountered an invalid operation error: {ex.Message}");
            return new UnprocessableEntityObjectResult($"{ex.Message}");
        }
        catch (UnauthorizedAccessException ex)
        {
            logger.LogError(ex, $"{LoggingConstants.HskUiLogPrefix} {nameof(GetCaseWitnessesLegacy)} function encountered an unauthorized access error: {ex.Message}");
            return new UnauthorizedObjectResult($"{nameof(GetCaseWitnessesLegacy)} error: {ex.Message}");
        }
        catch (Exception ex)
        {
            logger.LogError($"{LoggingConstants.HskUiLogPrefix} {nameof(GetCaseWitnessesLegacy)} function encountered an error: {ex.Message}");

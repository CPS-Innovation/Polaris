// <copyright file="RenameMaterialLegacy.cs" company="TheCrownProsecutionService">
...
/// </summary>
/// <remarks>
/// Initializes a new instance of the <see cref="RenameMaterialLegacy"/> class.
/// </remarks>
...
public class RenameMaterialLegacy(
    ILogger<RenameMaterialLegacy> logger,
    ICommunicationService communicationService,
    RenameMaterialRequestValidator renameMaterialRequestValidator) : BaseFunction(logger)
{
    private readonly ILogger<RenameMaterialLegacy> logger = logger;
    ...
    [OpenApiOperation(operationId: "RenameMaterialLegacy", tags: ["Material"], Description = "Represents a function that renames the material name/subject.")]
    ...
    [Function("RenameMaterialLegacy")]
    public async Task<IActionResult> Run([HttpTrigger(AuthorizationLevel.Anonymous, "patch", Route = RestApi.RenameMaterialLegacy)] HttpRequest request, int caseId, int materialId, CancellationToken cancellationToken = default)
    {
        try
        {
            var stopwatch = Stopwatch.StartNew();
            this.logger.LogInformation($"{LoggingConstants.HskUiLogPrefix} RenameMaterialLegacy function processed a request.");
            ...
            {
                this.logger.LogError($"{LoggingConstants.HskUiLogPrefix} caseId [{caseId}] RenameMaterialLegacy function failed in [{stopwatch.Elapsed}]");
                return new UnprocessableEntityObjectResult(result);
            }

            this.logger.LogInformation($"{LoggingConstants.HskUiLogPrefix} Milestone: caseId [{caseId}] RenameMaterialLegacy function completed in [{stopwatch.Elapsed}]");
        ...
        {
            this.logger.LogError(ex, $"{LoggingConstants.HskUiLogPrefix} RenameMaterialLegacy function encountered unsupported content type.");
            return new UnprocessableEntityObjectResult($"Rename error: {ex.Message}");
        }
        catch (Exception ex)
        {
            this.logger.LogError($"{LoggingConstants.HskUiLogPrefix} RenameMaterialLegacy function encountered an error: {ex.Message}");

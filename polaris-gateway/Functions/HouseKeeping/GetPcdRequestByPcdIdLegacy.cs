// <copyright file="GetPcdRequestByPcdIdLegacy.cs" company="TheCrownProsecutionService">
...
/// <summary>
/// Represents a function that return PCD Request information by case id and PCD id (legacy, URN-based route),
/// intended to be accessed via the Housekeeping UI front-end.
/// </summary>
/// <remarks>
/// Initializes a new instance of the <see cref="GetPcdRequestByPcdIdLegacy"/> class.
/// </remarks>
/// <param name="logger">The logger instance used to log information and errors.</param>
/// <param name="communicationService">The service used to get call pcd request service.</param>
public class GetPcdRequestByPcdIdLegacy(
    ILogger<GetPcdRequestByPcdIdLegacy> logger,
    ICommunicationService communicationService) : BaseFunction(logger)
{
    private readonly ILogger<GetPcdRequestByPcdIdLegacy> logger = logger;
    ...
    [OpenApiOperation(operationId: nameof(GetPcdRequestByPcdIdLegacy), tags: ["Pre-Charge Decision"], Description = "Returns PCD Request by PCD ID along with Case Id.")]
    ...
    [OpenApiRequestBody("application/json", typeof(PcdRequestDto), Description = "Body containing the PCD Request info.", Example = typeof(GetPcdRequestByPcdIdLegacy))]
    ...
    [Function(nameof(GetPcdRequestByPcdIdLegacy))]
    public async Task<IActionResult> Run(
        [HttpTrigger(AuthorizationLevel.Anonymous, "get", Route = RestApi.PcdRequestLegacy)] HttpRequest request, int caseId, int pcdId, CancellationToken cancellationToken = default)
    {
        try
        {
            var stopwatch = Stopwatch.StartNew();
            this.logger.LogInformation($"{LoggingConstants.HskUiLogPrefix} GetPcdRequestByPcdIdLegacy function processed a request.");
            ...
            if (result == null)
            {
                this.logger.LogError($"{LoggingConstants.HskUiLogPrefix} caseId [{caseId}] GetPcdRequestByPcdIdLegacy function failed in [{stopwatch.Elapsed}]");
                return new UnprocessableEntityObjectResult(result);
            }

            this.logger.LogInformation($"{LoggingConstants.HskUiLogPrefix} Milestone: caseId [{caseId}] GetPcdRequestByPcdIdLegacy function completed in [{stopwatch.Elapsed}]");
        ...
        {
            this.logger.LogError(ex, $"{LoggingConstants.HskUiLogPrefix} GetPcdRequestByPcdIdLegacy function encountered unsupported content type.");
            return new UnprocessableEntityObjectResult($"GetPcdRequestByPcdIdLegacy error: {ex.Message}");
        }
        catch (UnauthorizedAccessException ex)
        {
            this.logger.LogError(ex, $"{LoggingConstants.HskUiLogPrefix} GetPcdRequestByPcdIdLegacy function encountered UnauthorizedAccess Exception.");
            return new UnauthorizedObjectResult($"GetPcdRequestByPcdIdLegacy error: {ex.Message}");
        }
        catch (Exception ex)
        {
            this.logger.LogError($"{LoggingConstants.HskUiLogPrefix} GetPcdRequestByPcdIdLegacy function encountered an error: {ex.Message}");

// <copyright file="GetCaseDefendantsLegacy.cs" company="TheCrownProsecutionService">
...
/// </summary>
/// <remarks>
/// Initializes a new instance of the <see cref="GetCaseDefendantsLegacy"/> class.
/// </remarks>
/// <param name="logger">The logger instance used to log information and errors.</param>
/// <param name="caseDefendantsService">The service used to process the request and generate the result.</param>
public class GetCaseDefendantsLegacy(
            ILogger<GetCaseDefendantsLegacy> logger, 
            ICaseDefendantsService caseDefendantsService) : BaseFunction(logger)
{
    private readonly ILogger<GetCaseDefendantsLegacy> logger = logger;
    ...
    [OpenApiOperation(operationId: "GetCaseDefendantsLegacy", tags: ["Case"], Description = "Represents a function that retrieves the case defendants for display purposes.")]
    ...
    [Function("GetCaseDefendantsLegacy")]
    public async Task<IActionResult> Run([HttpTrigger(AuthorizationLevel.Anonymous, "get", Route = RestApi.CaseDefendantsLegacy)] HttpRequest request, int caseId, CancellationToken cancellationToken = default)
    {
        try
        {
            var stopwatch = Stopwatch.StartNew();
            this.logger.LogInformation($"{LoggingConstants.HskUiLogPrefix} GetCaseDefendantsLegacy function processed a request.");
            ...
            try
            {
                ...
                this.logger.LogInformation($"{LoggingConstants.HskUiLogPrefix} Milestone: caseId [{caseId}] GetCaseDefendantsLegacy function completed in [{stopwatch.Elapsed}]");
            ...
            {
                this.logger.LogError($"{LoggingConstants.HskUiLogPrefix} GetCaseDefendantsLegacy function encountered an error fetching case defendants for caseId [{caseId}]: {ex.Message}");
                throw new UnprocessableEntityException($"GetCaseDefendantsLegacy function encountered an error fetching case defendants for caseId [{caseId}]");
            }
        }
        catch (UnprocessableEntityException ex)
        {
            this.logger.LogError($"{LoggingConstants.HskUiLogPrefix} GetCaseDefendantsLegacy function encountered an unprocessable entity error: {ex.Message}");
        ...
        {
            this.logger.LogError($"{LoggingConstants.HskUiLogPrefix} GetCaseDefendantsLegacy function encountered an error: {ex.Message}");

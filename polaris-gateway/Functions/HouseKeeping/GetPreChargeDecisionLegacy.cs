// <copyright file="GetPreChargeDecisionLegacy.cs" company="TheCrownProsecutionService">
...
/// </summary>
/// <remarks>
/// Initializes a new instance of the <see cref="GetPreChargeDecisionLegacy"/> class.
/// </remarks>
/// <param name="logger">The logger instance used to log information and errors.</param>
/// <param name="communicationService">The service used to get call case history service.</param>

public class GetPreChargeDecisionLegacy(
    ILogger<GetPreChargeDecisionLegacy> logger,
    ICommunicationService communicationService) : BaseFunction(logger)
{
    private readonly ILogger<GetPreChargeDecisionLegacy> logger = logger;
    ...
    [OpenApiOperation(operationId: nameof(GetPreChargeDecisionLegacy), tags: ["CaseHistory"], Description = "Returns PCD with Case Id.")]
    ...
    [Function(nameof(GetPreChargeDecisionLegacy))]
    public async Task<IActionResult> Run(
        [HttpTrigger(AuthorizationLevel.Anonymous, "get", Route = RestApi.PreChargeDecisionLegacy)] HttpRequest request, int caseId, CancellationToken cancellationToken = default)
    {
        try
        {
            var stopwatch = Stopwatch.StartNew();
            this.logger.LogInformation($"{LoggingConstants.HskUiLogPrefix} GetPreChargeDecisionLegacy function processed a request.");
            ...
            if (result == null)
            {
                this.logger.LogError($"{LoggingConstants.HskUiLogPrefix} caseId [{caseId}] GetPreChargeDecisionLegacy function failed in [{stopwatch.Elapsed}]");
                return new UnprocessableEntityObjectResult(result);
            }

            this.logger.LogInformation($"{LoggingConstants.HskUiLogPrefix} Milestone: caseId [{caseId}] GetPreChargeDecisionLegacy function completed in [{stopwatch.Elapsed}]");
        ...
        {
            this.logger.LogError(ex, $"{LoggingConstants.HskUiLogPrefix} GetPreChargeDecisionLegacy function encountered unsupported content type.");
            return new UnprocessableEntityObjectResult($"GetPreChargeDecisionLegacy error: {ex.Message}");
        }
        catch (UnauthorizedAccessException ex)
        {
            this.logger.LogError(ex, $"{LoggingConstants.HskUiLogPrefix} GetPreChargeDecisionLegacy function encountered UnauthorizedAccess Exception.");
            return new UnauthorizedObjectResult($"GetPreChargeDecisionLegacy error: {ex.Message}");
        }
        catch (Exception ex)
        {
            this.logger.LogError($"{LoggingConstants.HskUiLogPrefix} GetPreChargeDecisionLegacy function encountered an error: {ex.Message}");

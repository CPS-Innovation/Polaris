// <copyright file="GetInitialReviewLegacy.cs" company="TheCrownProsecutionService">
...
/// </summary>
/// <remarks>
/// Initializes a new instance of the <see cref="GetInitialReviewLegacy"/> class.
/// </remarks>
...
public class GetInitialReviewLegacy(
    ILogger<GetInitialReviewLegacy> logger,
    ICommunicationService communicationService) : BaseFunction(logger)
{
    private readonly ILogger<GetInitialReviewLegacy> logger = logger;
    ...
    [OpenApiOperation(operationId: nameof(GetInitialReviewLegacy), tags: ["CaseHistory"], Description = "Returns case history initial review with Case Id.")]
    ...
    [Function(nameof(GetInitialReviewLegacy))]
    public async Task<IActionResult> Run(
        [HttpTrigger(AuthorizationLevel.Anonymous, "get", Route = RestApi.InitialReviewByCaseLegacy)] HttpRequest request, int caseId, CancellationToken cancellationToken = default)
    {
        try
        {
            var stopwatch = Stopwatch.StartNew();
            this.logger.LogInformation($"{LoggingConstants.HskUiLogPrefix} GetInitialReviewLegacy function processed a request.");
            ...
            if (result == null)
            {
                this.logger.LogError($"{LoggingConstants.HskUiLogPrefix} caseId [{caseId}] GetInitialReviewLegacy function failed in [{stopwatch.Elapsed}]");
                return new UnprocessableEntityObjectResult(result);
            }

            this.logger.LogInformation($"{LoggingConstants.HskUiLogPrefix} Milestone: caseId [{caseId}] GetInitialReviewLegacy function completed in [{stopwatch.Elapsed}]");
        ...
        {
            this.logger.LogError(ex, $"{LoggingConstants.HskUiLogPrefix} GetInitialReviewLegacy function encountered unsupported content type.");
            return new UnprocessableEntityObjectResult($"GetInitialReviewLegacy error: {ex.Message}");
        }
        catch (UnauthorizedAccessException ex)
        {
            this.logger.LogError(ex, $"{LoggingConstants.HskUiLogPrefix} GetInitialReviewLegacy function encountered UnauthorizedAccess Exception.");
            return new UnauthorizedObjectResult($"GetInitialReviewLegacy error: {ex.Message}");
        }
        catch (Exception ex)
        {
            this.logger.LogError($"{LoggingConstants.HskUiLogPrefix} GetInitialReviewLegacy function encountered an error: {ex.Message}");

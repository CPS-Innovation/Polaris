...
namespace PolarisGateway.Tests.Clients.PolarisPipeline
{
    public class SearchIndexClientTests
    {
        ...
        public async Task Query_ResultsAreOrderedByDocumentId()
        {
            var responseMock = new Mock<Response>();
            var fakeSearchLines = _fixture.CreateMany<SearchLine>(3).ToList();
            fakeSearchLines[0].DocumentId = 1111;
            fakeSearchLines[1].DocumentId = 2222;
            fakeSearchLines[2].DocumentId = 3333;
            ...
            {
                results.Count.Should().Be(3);
                results[0].DocumentId.Should().Be(3333);
                results[1].DocumentId.Should().Be(2222);
                results[2].DocumentId.Should().Be(1111);
            }
        }

...
namespace PolarisGateway.Functions.PolarisPipeline
{
    public class PolarisPipelineCancelCheckoutDocument : BasePolarisFunction
    {
        ...
        public async Task<IActionResult> Run(
            [HttpTrigger(AuthorizationLevel.Anonymous, "delete", Route = RestApi.DocumentCheckout)] HttpRequest req, string caseUrn, int caseId, Guid documentId)
        {
            ...
            try
            {
                ...
                _logger.LogMethodFlow(currentCorrelationId, loggingName, $"Cancel checkout of document with urn {caseUrn}, caseId {caseId}, id {documentId}");
                await _pipelineClient.CancelCheckoutDocumentAsync(caseUrn, caseId, documentId, request.CmsAuthValues, currentCorrelationId);

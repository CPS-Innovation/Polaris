...
import {
  CMS_AUTH_VALUES_SESSION_KEY,
  EXPERIMENTAL_REQUEST_CMSCOOKIES_RETURNED_TO_UI_PARAM_NAME,
...
const buildRedirectUrls = (jsonpCallbackFunction: string, correlationId: string | null) => {
  ...
  const shouldUseStoredHeader = !isSameSiteDomain();
  if (shouldUseStoredHeader) {
    inboundUrl += `?${EXPERIMENTAL_REQUEST_CMSCOOKIES_RETURNED_TO_UI_PARAM_NAME}=true`;
  }
...
const tryReauth = async (reauthUrls: string[], jsonpCallbackFunction: string): Promise<InSituReauthResult> => {
  ...
  console.log("Reauth result", reauthUrl, result);

  if (result.cmsAuthValues) {
    sessionStorage.setItem(CMS_AUTH_VALUES_SESSION_KEY, result.cmsAuthValues);
  }

...
namespace pdf_redactor.integration.tests.Clients
{
    public class PdfRedactorClient : IPdfRedactorClient
    {
        ...
        public async Task<Stream> RedactPdfAsync(RedactPdfRequestWithDocumentDto redactPdfRequest)
        {
            ...
            var redactRequest = _requestFactory.Create(HttpMethod.Put, $"/api/{RestApi.GetRedactPdfPathLegacy("pdf-redactor-integration-tests-urn", 1234, "pdf-redactor-integration-tests-documentId", 4567)}", currentCorrelationId);
        ...
        public async Task<Stream> RemoveOrRotateDocumentPagesAsync(ModifyDocumentWithDocumentDto removeDocumentPagesWithDocument)
        {
            ...
            var redactRequest = _requestFactory.Create(HttpMethod.Post, $"/api/{RestApi.GetModifyDocumentPathLegacy("pdf-redactor-integration-tests-urn", 1234, "pdf-redactor-integration-tests-documentId", 4567)}", currentCorrelationId);
            redactRequest.Content = requestMessage;

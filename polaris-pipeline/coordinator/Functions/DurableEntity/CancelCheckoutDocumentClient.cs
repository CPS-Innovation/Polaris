...
namespace coordinator.Functions.DurableEntity
{
    public class CancelCheckoutDocumentClient : BaseClient
    {
            ...
            ILogger log)
        {
            ...
            try
            {
                var response = await GetTrackerDocument(req, client, caseId, new PolarisDocumentId(polarisDocumentId));
            ...
            {
                log.LogMethodError(currentCorrelationId, nameof(CancelCheckoutDocumentClient), ex.Message, ex);

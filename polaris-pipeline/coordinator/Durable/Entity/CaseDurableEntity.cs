...
namespace coordinator.Durable.Entity
{
    ...
    {
        public static EntityId GetInstanceId(string caseId)
        {
            return new EntityId(
                nameof(CaseDurableEntity),
                RefreshCaseOrchestrator.GetKey(caseId)
            );
        }

        public static ICaseDurableEntity GetEntityProxyInterface(IDurableOrchestrationContext context, long caseId)
        {
            var entityId = GetInstanceId(caseId.ToString());
            return context.CreateEntityProxy<ICaseDurableEntity>(entityId);
        }

        public static async Task<CaseDurableEntity> GetReadOnlyEntityState(IDurableEntityClient client, string caseId)
        {
            var entityId = GetInstanceId(caseId);
            var response = await client.ReadEntityStateAsync<CaseDurableEntity>(entityId);
        ...
        public async Task<int> IncrementVersion()
        {
        ...
        public List<BaseDocumentEntity> AllDocuments() =>
            CmsDocuments.Cast<BaseDocumentEntity>()
                .Concat(PcdRequests.Cast<BaseDocumentEntity>())
                .Append(DefendantsAndCharges)
                .ToList();

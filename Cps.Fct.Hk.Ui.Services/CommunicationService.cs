    ...
    : ICommunicationService
{
    ...
    /// <inheritdoc/>
    public async Task<IReadOnlyCollection<PcdRequestDto>> GetPcdRequestOverview(int caseId, CmsAuthValues cmsAuthValues)
    {
        string caseIdString = caseId.ToString(CultureInfo.InvariantCulture);

        try
        {
            this.logger.LogInformation($"{LoggingConstants.HskUiLogPrefix} Fetching PCD Requests overview for caseId [{caseIdString}].");
            Guid correlationId = cmsAuthValues.CorrelationId == Guid.Empty ? Guid.NewGuid() : cmsAuthValues.CorrelationId;

            var request = new GetPcdRequestsOverviewRequest(caseId, correlationId);
            IReadOnlyCollection<PcdRequestDto> unusedMaterials = await this.apiClient.GetPcdRequestOverviewAsync(request, cmsAuthValues).ConfigureAwait(false);

            return unusedMaterials;
        }
        catch (Exception ex)
        {
            this.logger.LogError(ex, $"{LoggingConstants.HskUiLogPrefix} Error occurred while fetching PCD Requests overview information for caseId [{caseIdString}]");
            this.logger.LogError(ex, ex.Message);
            throw;
        }
    }
    ...
    /// <inheritdoc cref="ICommunicationService.SetMaterialReadStatusAsync(int, SetMaterialReadStatusType, CmsAuthValues, Guid)"/>
    public async Task<SetMaterialReadStatusResponse> SetMaterialReadStatusAsync(int materialId, SetMaterialReadStatusType action, CmsAuthValues cmsAuthValues, Guid correspondenceId = default)
    {
        try
        {
    ...
    /// <inheritdoc/>
    public async Task<UpdateExhibitResponse> UpdateExhibitAsync(int caseId, EditExhibitRequest exhibit, CmsAuthValues cmsAuthValues, Guid correspondenceId = default)
    {
        try
        {
    ...
    /// <inheritdoc/>
    public async Task<UpdateStatementResponse> UpdateStatementAsync(int caseId, EditStatementRequest statement, CmsAuthValues cmsAuthValues, Guid correspondenceId = default)
    {
        try
        {
